...
syn region  tjpstring	start=+`+ skip=+\\'+ end=+'+ contains=tjparch 
syn region  tjpstring	start=+`+ skip=+\\'+ end=+`+ contains=tjparch 
syn keyword tjpKeywords task effort depends macro project timeformat currency scenario flags rate vacation resource account start end duration headline columns loadunit hidetask accumulate scenarios caption cost revenue efficiency complete allocate
syn match tjpReports "\<[a-z]\+report\>"
syn match tjpCalendars "\<[a-z]\+calendar\>"
syn match tjpNumber "\<\d\+\(\.\d\+\)\?\>"
syn match tjpTime "\<\d\+\(\.\d\+\)\?[hdwm]\>"
syn match tjpDate "\<\d\{4}-\d\{1,2}-\d\{1,2}\>"
syn match tjpMacro "${\w\+}"
syn match tjpOpen "{\|}\|\[\|\]\|!"
syn match tjpInclude $include "\w\+\.tji"$
...
"  HiLink tjpjump		Include
  ...
  HiLink tjpKeywords   Function
  HiLink tjpReports    Function
  HiLink tjpCalendars  Function
  HiLink tjpNumber     Number
  HiLink tjpTime       Number
  HiLink tjpDate       Number
  HiLink tjpMacro      Special
  HiLink tjpOpen       Delimiter
  HiLink tjpInclude    Keyword
...
highlight Delimiter None guifg=Blue
highlight Number None guifg=Cyan
highlight String None guifg=DarkYellow
